...
from oes.email.config import Config, EmailSenderType, SMTPSettings
from oes.email.types import Email
from typing_extensions import TypeAlias

EmailSender: TypeAlias = Callable[[Email, Config], Awaitable]
...
async def mock_email_sender(email: Email, settings: Config):
    """Mock email sender."""
    logger.info(
        "Mock sending email:\n"
        "\n"
...
async def smtp_email_sender(email: Email, settings: Config):
    """SMTP email sender."""
...
async def mailgun_email_sender(email: Email, settings: Config):
    """Mailgun API."""
