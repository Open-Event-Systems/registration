...
from oes.email.html import process_html
from oes.email.types import Attachment, AttachmentType


@frozen
class RenderResult:
    """The result of rendering an email."""
...
class Attachments:
    ...
    def add(
        ...
        self._attachments[id_] = Attachment(
            ...
            attachment_type=(
                AttachmentType.inline if inline else AttachmentType.attachment
            ),
        )
...
def render_message(
    ...
    return RenderResult(ns.to, getattr(ns, "from"), ns.subject, text, html)
...
def render_template(
    ...
    result = tmpl.render({**data, "email": namespace})
